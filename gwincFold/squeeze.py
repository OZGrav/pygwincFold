...
def sql(ifo):
    ...
    FMm = ifo.FoldingMirrorSuspension.Stage[0].Mass
    mass = 1/sqrt((1/m)**2 + (1/FMm)**2)
    Titm = ifo.Optics.ITM.Transmittance
    Tsrm = ifo.Optics.SRM.Transmittance
    tSR = sqrt(Tsrm)
    rSR = sqrt(1 - Tsrm)
    fSQL = (1/(2*pi))*(8/c)*sqrt((power.parm*w0)/(mass*Titm))*(tSR/(1+rSR))
    return fSQL


def computeFCParams(ifo, f_sql=None):
