...
def precomp_suspension(f, ifo):
    ...
    if 'VHCoupling' in ifo.Suspension:
        ...
        pc.VHCoupling.theta = ifo.Infrastructure.FoldedLength / const.R_earth
    hForce, vForce, hTable, vTable, tst_suscept = suspQuad(
        f, ifo.Suspension)
    hForce_f, vForce_f, hTable_f, vTable_f, tst_suscept_f = suspQuad(
        f, ifo.FoldingMirrorSuspension)
    pc.hForce = hForce
    pc.vForce = vForce
    pc.hTable = hTable
    pc.vTable = vTable
    pc.tst_suscept = tst_suscept
    pc.hForce_f = hForce_f
    pc.vForce_f = vForce_f
    pc.hTable_f = hTable_f
    pc.vTable_f = vTable_f
    pc.tst_suscept_f = tst_suscept_f
    return pc


def precomp_suspension_f(f, ifo):
    pc = Struct()
    pc.VHCoupling = Struct()
    if 'VHCoupling' in ifo.Suspension:
        pc.VHCoupling.theta = ifo.Suspension.VHCoupling.theta
