...
def shotradSignalRecycled(f, ifo, sustf, power):
    ...
    # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    lambda_arm = 1 - (1 - ifo.Optics.Loss)**2 * (1 - ifo.Optics.ETM.Transmittance)* (1 - ifo.Optics.FM.Transmittance)
    ...
    test_mass_suscept = (sustf.tst_suscept + sustf.tst_suscept_f)
    h_SQL = sqrt((8 * hbar * np.abs(test_mass_suscept))) / L  # SQL Strain
    K = - 16 * P * omega_0 / c**2 * test_mass_suscept
