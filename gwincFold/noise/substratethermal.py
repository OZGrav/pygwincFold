...
class SubstrateBrownian(nb.Noise):
    ...
    def calc(self):
        ...
        nFM = substrate_brownian_Fold(
            self.freq, self.ifo.Materials, cavity.wBeam_FM, self.ifo.Infrastructure.FoldAngle, self.ifo.Laser.Wavelength)
        return (nITM + nETM+2*nFM) * 2
...
class SubstrateThermoElastic(nb.Noise):
    ...
    def calc(self):
        ...
        nETM = substrate_thermoelastic(
            self.freq, self.ifo.Materials, cavity.wBeam_ETM)
        nFM = substrate_thermoelastic_Fold(
            self.freq, self.ifo.Materials, cavity.wBeam_FM, self.ifo.Infrastructure.FoldAngle, self.ifo.Laser.Wavelength)
        return (nITM + nETM + 2*nFM) * 2
...
def substrate_brownian_Fold(f, materials, wBeam, fold_angle, wavelength):
    """Substrate thermal displacement noise spectrum due to substrate mechanical loss
    ...
    """
    ks = 2*np.pi/wavelength * fold_angle
    r0 = wBeam/np.sqrt(2)
    Y = materials.FoldingMirrorSubstrate.MirrorY
    sigma = materials.FoldingMirrorSubstrate.MirrorSigma
    c2 = materials.FoldingMirrorSubstrate.c2
    n = materials.FoldingMirrorSubstrate.MechanicalLossExponent
    alphas = materials.FoldingMirrorSubstrate.Alphas
    kBT = const.kB * materials.FoldingMirrorSubstrate.Temp

    cftm, aftm = substrate_brownian_FiniteCorr_Fold(materials, wBeam)
    stripe = stripe_corr(ks*r0/np.sqrt(2)) 
    ...
    return (csurf + cbulk)*stripe


def stripe_corr(b):
    return 1 + 2*np.exp(-3*b**2/8)*scipy.special.i0(b**2/8)+0.5*np.exp(-1*b**2) + 0.5*np.exp(-1*b**2/2)*scipy.special.i0(b**2/2)


def substrate_brownian_FiniteCorr_Fold(materials, wBeam):
    ...
    """
    a = materials.FoldingMirrorMassRadius
    h = materials.FoldingMirrorMassThickness
    Y = materials.FoldingMirrorSubstrate.MirrorY
    sigma = materials.FoldingMirrorSubstrate.MirrorSigma
...
def substrate_thermoelastic_Fold(f, materials, wBeam, fold_angle, wavelength):
    """Substrate thermal displacement noise spectrum from thermoelastic fluctuations
    ...
    """
    sigma = materials.FoldingMirrorSubstrate.MirrorSigma
    rho = materials.FoldingMirrorSubstrate.MassDensity
    kappa = materials.FoldingMirrorSubstrate.MassKappa # thermal conductivity
    alpha = materials.FoldingMirrorSubstrate.MassAlpha # thermal expansion
    CM = materials.FoldingMirrorSubstrate.MassCM # heat capacity @ constant mass
    Temp = materials.FoldingMirrorSubstrate.Temp # temperature
    kBT = const.kB * materials.FoldingMirrorSubstrate.Temp
    r0 = wBeam/np.sqrt(2)
    ks = 2*np.pi/wavelength * fold_angle
    ...
    # Corrections for finite test masses:
    S *= substrate_thermoelastic_FiniteCorr_Fold(materials, wBeam)

    # Stripe Correction
    S *= (1+(ks*r0/np.sqrt(2*np.pi)))
...
def substrate_thermoelastic_FiniteCorr_Fold(materials, wBeam):
    """Substrate thermoelastic noise finite-size test mass correction
