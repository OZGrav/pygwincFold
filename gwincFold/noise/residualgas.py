...
from .. import Struct
from .. import nb
from ..ifo.noises import dhdl, arm_cavity, qITM, qETM
...
def ResidualGasDamping_constructor_Fold(species_name):
    """Reisidual gas damping for a single species

    """

    class GasDampingSpecies(nb.Noise):
        name = 'Damping' + species_name +"FM"
        style = dict(
            label=RESGAS_STYLES[species_name]['label'] + ' damping',
            color=RESGAS_STYLES[species_name]['color'],
            linestyle='--',
        ...
        @nb.precomp(sustf=precomp_suspension_f)
        def calc(self, sustf):
            rg = self.ifo.Infrastructure.ResidualGas
            species = rg[species_name]
            squeezed_film = rg.get('SqueezedFilm', Struct())
            ...
            if squeezed_film is None:
                ...
                n = 4 * residual_gas_damping_test_mass_Fold(
                    self.freq, self.ifo, species, sustf, squeezed_film)
...
class ResidualGas(nb.Budget):
    ...
    noises = [
        ...
        ResidualGasDamping_constructor_Fold('H2'),
        ResidualGasDamping_constructor_Fold('N2'),
        ResidualGasDamping_constructor_Fold('H2O'),
        ResidualGasDamping_constructor_Fold('O2')
    ]



def beam_size(z, ifo):
    RcITM = ifo.Optics.Curvature.ITM
    RcETM = ifo.Optics.Curvature.ETM
    RcFM = ifo.Optics.Curvature.FM
    wavelength = ifo.Laser.Wavelength
    fmpos = ifo.Infrastructure.FoldedLength
    length = fmpos
    full_length = ifo.Infrastructure.Length
    q1 = qITM(RcITM, RcETM, RcFM, length)
    q2 = qETM(RcITM, RcETM, RcFM, length)
    out1 = np.where(z<=fmpos, np.sqrt(wavelength/np.pi*(np.abs(q1+z))**2/np.imag(q1+z)),0)
    out2 = np.where(z>fmpos, np.sqrt(wavelength/np.pi*(np.abs(q2-z+full_length))**2/np.imag((q2-z+full_length))),0)
    return out1+out2
...
def residual_gas_damping_test_mass_Fold(f, ifo, species, sustf, squeezed_film):
    """Noise due to residual gas damping for one test mass
    ...
    :returns: displacement noise
    """
    sus = ifo.FoldingMirrorSuspension
    if 'Temp' in sus.Stage[0]:
        kT = sus.Stage[0].Temp * const.kB
    else:
        kT = sus.Temp * const.kB
    ...
    mass = species.mass 
    radius = ifo.Materials.FoldingMirrorMassRadius
    thickness = ifo.Materials.FoldingMirrorMassThickness
    thermal_vel = sqrt(kT/mass)  # thermal velocity
