...
class CoatingBrownian(nb.Noise):
    ...
    def calc(self):
        ...
        FM = mirror_struct_Fold(self.ifo, 'FM')
        ...
        nFM = coating_brownian_Fold(
            self.freq, FM, wavelength, cavity.wBeam_FM
        )
...
class CoatingThermoOptic(nb.Noise):
    ...
    def calc(self):
        ...
        nFM, _, _, _ = coating_thermooptic_Fold(
            self.freq, FM, wavelength, cavity.wBeam_FM,
        )
        return (nITM + nETM + 2*nFM) * 2
...
def coating_brownian_Fold(f, mirror, wavelength, wBeam, power=None):
    """Coating brownian noise for a given collection of coating layers
...
def coating_thermooptic_Fold(f, mirror, wavelength, wBeam):
    """Optical coating thermo-optic displacement noise spectrum
    ...
    StoZ = SsurfT * gTO * dTO**2 * 1.5 
    SteZ = SsurfT * gTE * dTE**2 * 1.5
    StrZ = SsurfT * gTR * dTR**2 * 1.5
...
def mirror_struct_Fold(ifo, tm):
    ...
    # NOTE: we deepcopy this struct since we'll be modifying it (and
    # it would otherwise be stored by reference)
    sub = copy.deepcopy(ifo.Materials.FoldingMirrorSubstrate)
    coat = copy.deepcopy(ifo.Materials.FoldingMirrorCoating)
    rad = copy.deepcopy(ifo.Materials.FoldingMirrorMassRadius)
    thick = copy.deepcopy(ifo.Materials.FoldingMirrorMassThickness)
    mirror = Struct()
    mirror.Substrate = sub
    mirror.Coating = coat
    mirror.MassRadius = rad
    mirror.MassThickness = thick
    optic = ifo.Optics.get(tm)
    if 'CoatLayerOpticalThickness' in optic:
        mirror.Coating.dOpt = optic['CoatLayerOpticalThickness']
    else:
