...
from ..suspension import getJointParams, precomp_suspension, precomp_suspension_f
from .. import nb
...
def SuspensionThermal_constructor_Fold(stage_name, direction):
    """Suspension thermal for a single stage in one direction
    ...
    color_map = {
        "Top": "xkcd:electric purple",
        "UIM": "xkcd:electric blue",
        "PUM": "xkcd:bright green",
        "TM": "xkcd:bright yellow",
    }
    label_map = {
        "TM": "Test mass",
    }

    class SuspensionThermalStageF(nb.Noise):
        name = f"{direction.capitalize()}{stage_name}"
        style = dict(
            label=f"{direction.capitalize()}. {label_map.get(stage_name, stage_name)}"+"FM",
            color=color_map[stage_name],
            linestyle="-" if direction == "horiz" else "--",
            alpha=0.7,
        )
        ...
        def calc(self, sustf):
            n = susptherm_stage(
                self.freq, self.ifo.FoldingMirrorSuspension, sustf, stage_map[stage_name], direction)
            return abs(n) * 4

    return SuspensionThermalStageF
...
class SuspensionThermalFolding(nb.Budget):
    """Suspension Thermal

    """

    name = 'SuspensionThermalFolding'

    style = dict(
        label='Suspension Thermal FM',
        color='xkcd:marine blue',
    )

    noises = [
        SuspensionThermal_constructor_Fold("Top", "horiz"),
        SuspensionThermal_constructor_Fold("UIM", "horiz"),
        SuspensionThermal_constructor_Fold("PUM", "horiz"),
        SuspensionThermal_constructor_Fold("TM", "horiz"),
        SuspensionThermal_constructor_Fold("Top", "vert"),
        SuspensionThermal_constructor_Fold("UIM", "vert"),
        SuspensionThermal_constructor_Fold("PUM", "vert")
    ]
